...
Variable(;
    ...
    Variable(variability, T, size, value, 
        unit, displayUnit, min, max, start, 
        fixed, nominal, info, flow, state, property)
...
#    @show iv.fdef
    add_variable!(
        ...
        iv.name, 
        as_field_value(
            haskey(kwargs, iv.name) ? kwargs[iv.name] : iv.init(instance, time), time)
        )
end
