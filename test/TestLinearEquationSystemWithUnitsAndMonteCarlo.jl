...
simulate!(filterCircuit1, QBDF(autodiff = AutoFiniteDiff()), stopTime = 1.0, log=true)
plot(filterCircuit1, plotVariables, figure=1)

filterCircuit2 = @instantiateModel(FilterCircuit, unitless=false, logCode=logCode)
simulate!(filterCircuit2, QBDF(autodiff = AutoFiniteDiff()), stopTime = 1.0, log=true)
...
simulate!(filterCircuit3, QBDF(autodiff = AutoFiniteDiff()), stopTime = 1.0, log=true,
...
simulate!(filterCircuit4, QBDF(autodiff = AutoFiniteDiff()), stopTime = 1.0, log=true,
          merge = Map(R1=Map(R=(110.0∓10.0)u"Ω"),
                      R2=Map(R=(220.0∓20.0)u"Ω"),
                      C =Map(C=(2.6e-3∓1e-4)u"F")
                     )
...
simulate!(filterCircuit5, QBDF(autodiff = AutoFiniteDiff()), stopTime = 1.0, log=true,
          merge = Map(R1=Map(R=(110.0±10.0)u"Ω"),
                      R2=Map(R=(220.0±20.0)u"Ω"),
                      C =Map(C=(2.6e-3±1e-4)u"F")
                     )
...
simulate!(filterCircuit6, QBDF(autodiff = AutoFiniteDiff()), stopTime = 1.0, log=true,
          merge = Map(R1=Map(R=(110.0±10.0)u"Ω"),
                      R2=Map(R=(220.0±20.0)u"Ω"),
                      C =Map(C=(2.6e-3±1e-4)u"F")
                     )
