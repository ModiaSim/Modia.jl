...
@define Flange = Model(
    phi = potential,
    tau = flow,
    icon="Modelica.Mechanics.Rotational.Interfaces.Flange_a" 
)
...
@define Fixed = Model(
    flange = Flange,
    icon="Modelica.Mechanics.Rotational.Components.Fixed", 
    equations = :[
        flange.phi = 0.0]
)
...
@define Inertia = Model(
    ...
    icon="Modelica.Mechanics.Rotational.Components.Inertia", 
    equations = :[
        phi = flange_a.phi
        phi = flange_b.phi
        w = der(phi)
...
@define IdealGear = Model(
    ...
    icon="Modelica.Mechanics.Rotational.Components.IdealGear", 
    equations = :[
        phi_a = flange_a.phi
        phi_b = flange_b.phi
        phi_a = ratio * phi_b
...
@define PartialTorque = Model(
    tau = input,
    flange = Flange,
    icon="Modelica.Mechanics.Rotational.Interfaces.PartialTorque",     
)

# Input signal acting as external torque on a flange
@define Torque         = PartialTorque | Model(
    icon="Modelica.Mechanics.Rotational.Sources.Torque", 
...
@define Damper = Model(
    ...
    d        = 1.0u"N*m*s/rad", # (info = "Damping constant"),
    icon="Modelica.Mechanics.Rotational.Components.Damper", 
    equations = :[
        phi_rel      = flange_b.phi - flange_a.phi
        w_rel        = der(phi_rel)
        flange_b.tau = tau
...
@define PartialAbsoluteSensor = Model(
    flange   = Flange,
    icon="Modelica.Mechanics.Rotational.Interfaces.PartialAbsoluteSensor", 
    equations = :[flange.tau = 0] 
)

# Ideal sensor to measure the absolute flange angular velocity
@define AngleSensor         = PartialAbsoluteSensor | Model(phi = output, 
    icon="Modelica.Mechanics.Rotational.Sensors.AngleSensor", 
    equations = :[phi = flange.phi])

UnitlessAngleSensor = PartialAbsoluteSensor | Model(phi = output, equations = :[phi = flange.phi*u"1/rad"])

@define SpeedSensor         = PartialAbsoluteSensor | Model(w = output, 
    icon="Modelica.Mechanics.Rotational.Sensors.SpeedSensor", 
    equations = :[w = der(flange.phi)])
