...
@define Pin = Model( v = potential, i = flow, 
    icon="Modelica.Electrical.Analog.Interfaces.Pin" )

@define OnePort = Model( 
    p = Pin(annotation=:left), 
    n = Pin(annotation=:right), 
    icon="Modelica.Electrical.Analog.Interfaces.OnePort", 
    equations = :[
        v = p.v - n.v
        0 = p.i + n.i
        i = p.i ] 
...
`R` - Resistance Ω
"""  
@define Resistor = OnePort | Model( R = 1.0u"Ω", icon="Modelica.Electrical.Analog.Basic.Resistor", equations = :[ R*i = v ] )

# @showModel(Resistor)

#@define Capacitor = OnePort | Model( C = 1.0u"F", v=Var(init=0.0u"V"), equations = :[ C*der(v) = i ] ) 
@define Capacitor = OnePort | Model( C = 1.0u"F", v=Var(init=0.0), 
    icon="Modelica.Electrical.Analog.Basic.Capacitor", 
    equations = :[ C*der(v) = i ] ) 

#@define Inductor = OnePort | Model( L = 1.0u"H", i=Var(init=0.0u"A"), equations = :[ L*der(i) = v ] )
@define Inductor = OnePort | Model( L = 1.0u"H", i=Var(init=0), 
    icon="Modelica.Electrical.Analog.Basic.Inductor", 
    equations = :[ L*der(i) = v ] )

@define ConstantVoltage = OnePort | Model( V = 1.0u"V", 
    icon="Modelica.Electrical.Analog.Sources.ConstantVoltage",
    equations = :[ v = V ] )

@define ConstantCurrent = OnePort | Model( I = 1.0u"A",
    icon="Modelica.Electrical.Analog.Sources.ConstantCurrent",
    equations = :[ i = I ] )

@define Ground = Model( p = Pin, 
    icon="Modelica.Electrical.Analog.Basic.Ground",
    equations = :[ p.v = 0.0u"V" ] ) 
